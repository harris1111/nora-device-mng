...
export default function SystemListPage() {
  ...
  const [view, setView] = useState(() => localStorage.getItem('systemView') || 'grid');
  ...
  const fetchDevices = useCallback(() => {
    ...
    getDevices(apiParams)
      ...
      .catch(() => setError('Không thể tải danh sách hệ thống'))
  ...
  const handleViewChange = (newView: string) => {
    setView(newView);
    localStorage.setItem('systemView', newView);
  };
  ...
  const handleBulkDelete = async () => {
    ...
    if (!window.confirm(`Xóa ${selectedIds.size} hệ thống đã chọn? Thao tác này không thể hoàn tác.`)) return;
    try {
      await bulkDeleteDevices([...selectedIds]);
      fetchDevices();
    } catch {
      setError('Không thể xóa hệ thống');
    }
  };
  ...
  const resultSummary = loading
    ? 'Đang cập nhật danh sách hệ thống...'
    : total > 0
      ? `${total} hệ thống phù hợp${hasActiveFilters ? ' với bộ lọc hiện tại' : ''}`
      : hasActiveFilters
        ? 'Không có hệ thống phù hợp với bộ lọc hiện tại'
        : 'Chưa có hệ thống trong hệ thống';
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400 mb-0.5">Khám phá hệ thống</p>
          <h1 className="text-2xl font-extrabold text-slate-800 tracking-tight">Danh sách hệ thống</h1>
        ...
        <div className="flex shrink-0 flex-wrap items-center gap-2">
          ...
          {canExport && (
            <button
              ...
              title={total === 0 ? 'Không có hệ thống để xuất' : `Xuất tất cả ${total} hệ thống phù hợp với bộ lọc`}
            >
      ...
      {selectedIds.size > 0 && hasBulkActions && (
        <div className="animate-slide-up rounded-2xl border border-indigo-100 bg-gradient-to-r from-indigo-50 to-violet-50 px-4 py-4 shadow-sm sm:px-5">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <p className="text-sm font-semibold text-indigo-700">Đã chọn {selectedIds.size} hệ thống trên trang hiện tại</p>
      ...
      {!loading && !error && devices.length === 0 && (
        <EmptyState
          ...
          title={hasActiveFilters ? 'Không có hệ thống phù hợp với bộ lọc' : 'Chưa có hệ thống nào'}
          description={
            hasActiveFilters
              ? 'Thử thay đổi hoặc xóa bộ lọc để xem kết quả khác.'
              : 'Nhấn nút thêm ở góc phải (hoặc bên dưới) để bắt đầu tạo hệ thống quản lý đầu tiên.'
          }
        />
      )}
              ...
              <DeviceCard
                ...
                selectionControl={hasBulkActions ? (
                  <input
                    ...
                    aria-label={`Chọn hệ thống ${device.name}`}
