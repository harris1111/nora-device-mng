...
export default function DeviceForm({ initialData, onSubmit, submitLabel = 'Lưu' }) {
  ...
  useEffect(() => {
    getLocations().then(setLocations).catch(() => setError('Không thể tải danh sách vị trí. Vui lòng thử lại.'));
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      fd.append('managed_by', managedBy.trim());
      fd.append('owned_by', ownedBy.trim());
      ...
      <div className="card-glass border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] overflow-hidden">
        <div className="p-6 md:p-8 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <FormTextInput id="managed_by" label="Quản lý bởi" value={managedBy} onChange={setManagedBy} placeholder="Ví dụ: IT, Kỹ thuật" />
            <FormTextInput id="owned_by" label="Đang sử dụng bởi" value={ownedBy} onChange={setOwnedBy} placeholder="Ví dụ: ME, Phòng sản xuất" />
            ...
            <div className="space-y-1.5 md:col-span-2 pt-2 border-t border-slate-100 mt-2">
              <label htmlFor="image" className="block text-sm text-slate-700 font-semibold mb-2">Hình ảnh (Tùy chọn)</label>
              <div className="flex flex-col sm:flex-row gap-6 items-start sm:items-center">
                <div className={`relative w-40 h-40 shrink-0 border-2 border-dashed rounded-2xl flex flex-col items-center justify-center overflow-hidden transition-colors ${imagePreview ? 'border-transparent bg-slate-100' : 'border-slate-300 bg-slate-50 hover:bg-slate-100'}`}>
                  ...
                  <input id="image" type="file" accept="image/*" onChange={handleImageChange} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" />
                </div>
                <div className="flex-1">
                  <p className="text-sm text-slate-500 mb-3">Nhấn vào hộp hình để tải ảnh lên. Định dạng: JPG, PNG, WEBP.</p>
