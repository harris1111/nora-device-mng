 ...
 */
export default function PrintQrcodeButton({ deviceId, storeId }: Props) {
  const handlePrint = () => {
...
<div class="label">
  ...
  <span id="store-label"></span>
    ...
    const qrEl = doc.getElementById('qr') as HTMLImageElement | null;
    const labelEl = doc.getElementById('store-label');
    if (qrEl) qrEl.src = qrSrc;
    if (labelEl) labelEl.textContent = storeId;
