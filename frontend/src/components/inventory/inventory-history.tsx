...
export default function InventoryHistory({ deviceId, records, onUpdate }: Props) {
  ...
  const [editingAttachments, setEditingAttachments] = useState<InventoryAttachmentItem[]>([]);
  ...
  const [completeTarget, setCompleteTarget] = useState<InventoryRecord | null>(null);
  ...
  const openCompleteModal = (r: InventoryRecord) => {
    setCompleteTarget(r);
  ...
  const handleComplete = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await updateInventoryRecord(completeTarget.id, fd);
      closeCompleteModal();
      onUpdate?.();
    } catch (err: unknown) {
  ...
  const handleViewAttachment = (a: InventoryAttachmentItem) => {
    const url = inventoryAttachmentUrl(a.id);
  ...
  const handleRemoveExistingAttachment = async (id: string) => {
    ...
    try {
      await deleteInventoryAttachment(id);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (editingId) {
        await updateInventoryRecord(editingId, fd);
      } else {
        await createInventoryRecord(deviceId, fd);
      }
      resetForm();
      onUpdate?.();
    } catch (err: unknown) {
        ...
        <div className="space-y-0">
          {records.map((r, i) => (
            <div key={r.id} className="relative pl-8 pb-5 last:pb-0 group">
              ...
              <div className="absolute left-0 top-1 w-6 h-6 rounded-full bg-sky-100 border-2 border-sky-400 flex items-center justify-center">
