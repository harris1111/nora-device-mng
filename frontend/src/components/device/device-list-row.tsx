...
export default memo(function DeviceListRow({ device, selectable, selected, onToggleSelect, showImageCol = true }: Props) {
    ...
    <tr className={`hover:bg-indigo-50/30 transition-colors group ${selected ? 'bg-indigo-50/40' : ''}`}>
      ...
      {showImageCol && (
        <td className="w-16 px-6 py-4">
          {isSystem ? (
            <div className="w-12 h-12 bg-gradient-to-br from-slate-600 to-slate-800 rounded-xl shadow-sm border border-slate-500/20 overflow-hidden flex items-center justify-center">
      ...
      <td className="px-6 py-4">
        <Link to={detailUrl} className="text-slate-800 hover:text-indigo-600 font-bold block truncate max-w-xs transition-colors">
          {device.name}
        </Link>
      </td>
      <td className="px-6 py-4">
        <span className="inline-flex items-center rounded-full border border-slate-200 bg-slate-50 px-2.5 py-1 text-xs font-semibold text-slate-600 shadow-sm">
          {isSystem ? getSystemCategoryLabel(device.systemCategory) : getTypeName(device.type)}
        </span>
      </td>
      <td className="px-6 py-4">
