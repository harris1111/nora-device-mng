...
function getPageMeta(path: string): { title: string; eyebrow: string } {
  ...
  if (path.startsWith('/system-categories')) return { title: 'Quản lý loại hệ thống', eyebrow: 'Danh mục' };
...
export default function AppLayout({ children }: Props) {
  ...
  const canViewSystemCategories = useCan('system_categories', 'view');
  ...
  const allNavItems: NavItem[] = [
    ...
    },
    {
      ...
      visible: canViewSystemCategories,
      icon: (
        <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
