...
router.get('/', requirePermission('permissions', 'view'), async (_req: Request, res: Response) => {
  try {
    ...
    for (const p of permissions) {
      ...
      matrix[p.role][p.module] = { canView: p.canView, canCreate: p.canCreate, canUpdate: p.canUpdate, canDelete: p.canDelete, canExport: p.canExport };
    }
    res.json(matrix);
  } catch (err) {
...
router.put('/:role', requirePermission('permissions', 'update'), async (req: Request, res: Response) => {
  try {
    ...
    const modules = req.body as Record<string, { canView?: boolean; canCreate?: boolean; canUpdate?: boolean; canDelete?: boolean; canExport?: boolean }>;
    if (!modules || typeof modules !== 'object') return res.status(400).json({ error: 'Invalid request body' });

    const updates: Array<{ module: string; canView: boolean; canCreate: boolean; canUpdate: boolean; canDelete: boolean; canExport: boolean }> = [];
    ...
    await prisma.$transaction(
      updates.map((u) =>
        prisma.permission.upsert({
          ...
          update: { canView: u.canView, canCreate: u.canCreate, canUpdate: u.canUpdate, canDelete: u.canDelete, canExport: u.canExport },
          create: { role: targetRole, module: u.module, canView: u.canView, canCreate: u.canCreate, canUpdate: u.canUpdate, canDelete: u.canDelete, canExport: u.canExport },
        })
      )
    );
    ...
    const result: Record<string, { canView: boolean; canCreate: boolean; canUpdate: boolean; canDelete: boolean; canExport: boolean }> = {};
    for (const p of perms) {
      result[p.module] = { canView: p.canView, canCreate: p.canCreate, canUpdate: p.canUpdate, canDelete: p.canDelete, canExport: p.canExport };
    }
